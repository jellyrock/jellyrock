...
- **Disambiguate explicitly.** For dialogs, the canonical answer is the `source/utils/dialogs.bs` helper family (`showAlertDialog` / `showConfirmDialog` / `showChoiceDialog` / `showListDialog` / `showInfoDialog` / `showKeyboardDialog`) backed by `components/dialogs/`. There is no longer a second path: `SceneManager`'s dialog methods (`userMessage` / `showConfirmationDialog` / `dismissDialog`) and its shared `returnData` / `isDataReturned` / `isPending*` fields were **deleted** in the #288 phase-3 migration. `SceneManager.isDialogOpen()` survives as a QUERY only. If you find code describing the old shared-field mechanism, it is a comment about history — don't cite it as a live alternative. `OverviewDialog` is NOT legacy: it backs `showInfoDialog` and is a member of the family.
