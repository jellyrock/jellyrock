...
**Goal.** A meta-skill that audits another skill's *actual execution* from its Claude Code session transcript. It runs the mechanical extractor at [`extract-friction.cjs`](extract-friction.cjs) over the JSONL, producing JSON across **four dimensions** — friction findings (`repeated-command`, `failed-recovery`, `confusion-marker`, `lint-spam`, `changelog-edit-attempt`, `tasks-leakage`, `test-claim-without-evidence`, `hardware-claim-mismatch`, `permission-gap`), performance (clock time, tokens-per-model, cache-hit ratio, cost estimate), a model-fit profile, and the one dimension no extractor can mechanize (output-accuracy judgment) — then classifies findings by root cause, weighs perf anomalies, proposes concrete edits to the audited skill's SKILL.md or supporting infra, and reassesses whether the skill's `model:` pin still fits. The load-bearing trick is to **name the anti-pattern explicitly** in any proposed callout — name the specific bash invocation, phrasing, or tool sequence that failed — so future agents recognize the failure shape and skip to the right approach; a generic "be careful" callout isn't enough. Recommended after any non-trivial skill invocation — a friction-clean run can still be slow, token-bloated, or trip a permission prompt that should be allowlisted. Ships at the **opus** tier (with `effort: low`) because the classification, accuracy judgment, and model-fit calls are judgment-heavy while the extraction is mechanical and pushed down to the helper.

**Inputs.** `$ARGUMENTS`: a required skill name (e.g. `pr`, `log`, `catchup`, `runtime-triage`), optionally followed by `--session <id>` to override the default (the most-recent transcript containing that skill). Expects the JSONL transcripts reachable under `~/.claude/projects/<sanitized-cwd>/` (override with `--transcripts-dir`), the `extract-friction.cjs` helper present, and — for model-fit + reconciliation context — the audited skill's prior `AUDIT-LOG.md` if any.

**Outputs.**

- The mechanical extraction JSON — `findings` / `performance` / `modelFit` blocks, plus the output-accuracy judgment the auditor adds by hand.
- A classified, root-cause-grouped set of findings — noise dropped, duplicates merged; each retained finding carries a concrete fix that names the anti-pattern explicitly.
- Concrete edit proposals, each targeting ONE of: the audited skill's SKILL.md, a new helper script + its `.claude/settings.json` allowlist line, `.claude/settings.json`, or a load-bearing CLAUDE.md rule — each with root cause / target / exact diff / rationale / verification, ranked by severity and surfaced *before* applying.
- A model-fit recommendation (`keep | downgrade to Y | upgrade to Y | watch`).
- Applied approved edits (one diff per Edit call), an `AUDIT-LOG.md` section appended to the audited skill (always), and a `/log decision` only when an architectural trigger fires.
...
- The `performance` block is inspected even when `findings` is empty — perf anomalies (anomalous duration, token bloat, low cache-hit, broken cost) are findings too.
- Each `permission-gap` becomes a `.claude/settings.json` allowlist proposal for a project-owned script — mechanical, no judgment beyond confirming the script is repo-owned and non-destructive.
...
- The audited skill's `AUDIT-LOG.md` is updated; a `/log decision` is invoked only when an architectural trigger fires (new helper script, model change, skill retired, new hook, load-bearing rule change).
- The skill never auto-commits.

**Failure modes to avoid.**

- **Treating the raw extractor output as findings.** The detectors are recall-tuned; a grep'd string is not a real invocation. Confirm each before keeping it — e.g. a `confusion-marker` cluster may be legitimate questioning, not floundering.
- **Skipping the `performance` block when `findings` is empty.** The audit isn't complete until `performance` is inspected.
- **Recommending a model change off a single weak signal.** A lone audit is weak evidence — flag `watch` for the next audit unless the profile is unambiguous.
...
- **A run clean across all four dimensions** — no friction AND normal performance AND no permission-gaps AND correct first-try output. Auditing such a run pattern-matches noise. (A friction-clean run *with* a perf anomaly or a permission-gap *is* still auditable — that's the point of the four-dimension model.)
