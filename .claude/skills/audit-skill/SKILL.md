...
**Inputs.** `$ARGUMENTS`: a required skill name (e.g. `pr`, `log`, `catchup`, `runtime-triage`), optionally followed by a selection flag — `--last N` (scan the N most-recent sessions containing the skill), `--all` (every session), or `--session <id>` (one specific session); default is the single most-recent session. `--invocation latest|all` controls, per session, which invocation range(s) to read; `--whole-transcript` (with `--session`) scopes an isolated sub-agent transcript as one run — a sub-agent that self-invokes a skill never stamps `attributionSkill`, so the normal invocation filter finds nothing and the run is invisible without it. Expects the JSONL transcripts reachable under `~/.claude/projects/<sanitized-cwd>/`, the extractor present, and (for model-fit + reconciliation context) the audited skill's prior `AUDIT-LOG.md` if any.
...
- Concrete edit proposals targeting one of: the audited skill's SKILL.md, a new helper script + its allowlist line, `.claude/settings.json`, or an `AGENTS.md` rule — each with root cause / target / exact diff / rationale / verification, ranked by severity and surfaced before applying.
