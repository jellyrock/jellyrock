...
- On the **quick-fix** route only: a written plan file saved at `.claude/plans/focus-YYYY-MM-DD-<slug>.md` with a required `**Project:** <slug | n/a>` header line (the tracked project the plan advances, where the implementing session records the landing) and the standard sections — Context (the why — which banner / followup / pick surfaced the work), Approach (single recommended approach, not a menu), Critical files (scope envelope as a table), Verification (numbered checklist with JellyRock's standard npm wrappers), Landing & closeout (how the change reaches `main` and a release, and who lands each step, or `n/a — no production artifact`), What this plan deliberately does NOT do (scope boundaries) — rendered as the user's approval preview via the plan-mode dialog (the user edits the markdown inline before approving), followed by a copy-paste hand-off block naming the saved file: `/sonnet <plan-path>` for the procedural default, or an `Implement the plan at <path>` paste when the plan needs the judgment-grade tier (that line also asks for the landing record, because that route never runs `/sonnet`).
...
- A `Captures for /log` tail listing anything journal-worthy surfaced during triage or research that isn't yet journaled, using the types `/log` records. Omit if nothing surfaced; do not pad.
...
- The implementation tier is classified by `/focus` (default `/sonnet`; the judgment-grade tier only when a plan step needs in-flight judgment the plan can't pre-specify, or when the plan meets `/sonnet`'s "must not be a Sonnet run" list) and reflected in the hand-off block — the choice is made here, where the full plan context lives, not punted to the fresh session.
- **Any plan whose change must reach production to count as done has a filled-in `Landing & closeout` section** naming who lands each production step: the change lands by itself (and how to verify that it did), the implementing session runs it (only a routine, undoable step, with the exact command approved as part of the plan), or the operator runs it (the exact command, handed over to run right after the push). A followup is captured only if the operator chooses to defer. A plan with no production artifact writes `n/a — no production artifact`, so the omission is a choice, not a forgotten step.
...
- **Running a write skill in the planning session.** A skill with side effects and its own context budget doesn't belong in the `/focus` session: mixing it with planning blows the planning context. Plan via `/focus`, hand off, then invoke the write skill in the fresh session.
- **Punting the impl-model choice downstream.** Deciding the implementation tier (`/sonnet` vs the judgment-grade tier) is `/focus`'s call, made here where the full plan context lives — NOT deferred to the fresh session, which has *less* context and tends to default to the expensive tier by reflex. A fork-free "apply these edits, run these gates" plan is `/sonnet`; reserve the judgment-grade tier only for a step the plan cannot pre-specify (a perf rewrite shaped by a live run, an unknown-root-cause debug). A plan that meets `/sonnet`'s "must not be a Sonnet run" list (its last *When NOT to use* bullet) stays on the judgment-grade tier even when its edits look procedural; check that list rather than keeping a copy here.
- **Launching sub-agents by default instead of as an escape hatch** (quick-fix route). Inline research (a few targeted `Grep` + `Read` calls + `AskUserQuestion` on forks) is the right shape for the typical 1-3-area task. Sub-agents are the right shape ONLY when the work spans more than three distinct codebase areas AND each area has established conventions you'd need to deeply read multiple files in to honor.
- **Endpoint-inferred edit-site list** (quick-fix route). Listing the edit sites of a change that threads through a chain of layers (a new field passed from `source/api` through a Task node into a component, a value re-selected at each stage) by inspecting a few representative layers instead of enumerating them mechanically (grep for the sibling token the change runs parallel to, or trace the reference graph). It silently drops the intermediate pass-through layers, so the plan's Critical files ships incomplete and the gap surfaces only when the implementing session's `npm run validate` or tests fail.
...
1. **Research inline.** A few targeted `Grep` + `Read` calls to ground the approach in JellyRock's real conventions for the touched area. Inline research is the right shape for the typical 1–3-area task; only reach for a sub-agent when the work spans more than three distinct areas each needing deep reads. **List chain / pass-through edit sites mechanically:** when the change threads a new field or parameter parallel to an existing sibling (from `source/api` through a Task node into a component, say), enumerate the edit sites with `grep -rln '<sibling token>'` or by tracing the references, and build Critical files from that list, never from a few layers checked by hand.
...
3. **Draft the plan INTERNALLY**, then surface it via `EnterPlanMode` → `ExitPlanMode`. The plan-mode dialog is the SOLE preview surface — never render plan markdown to the conversation first. The plan opens with a `**Project:** <slug | n/a>` header line: the project under `docs/projects/` the plan advances, or `n/a` (`/sonnet` records the landing in that project's local PLAN). Sections: **Context** (the why — which pick / banner / followup surfaced it), **Approach** (a single recommended path, not a menu), **Critical files** (scope envelope as a table — CREATE / Edit / Delete per file), **Verification** (numbered checklist using JellyRock's npm wrappers: `npm run validate` for the BrightScript typecheck, `npm run test:scripts` for the vitest suite, `npm run test:tdd` for Roku device tests when in scope, `npm run lint` for the lint gate — never bare runners), **Landing & closeout** (who lands each step: most JellyRock changes land by themselves when their PR merges to `main`, and the draft release updates automatically; finalizing a release is a tag push, which is the operator's step; name any other step, such as a device check after merge, with the exact command and who runs it; or write `n/a — no production artifact`), **What this plan deliberately does NOT do** (scope boundaries).
4. **Save** the approved plan to `.claude/plans/focus-YYYY-MM-DD-<slug>.md` (the path `/sonnet` reads; gitignored, like `.claude/handoffs/`).
5. **Classify the implementation tier.** Default `/sonnet` for a fork-free procedural plan; reserve the judgment-grade tier only when a plan step needs in-flight judgment the plan can't pre-specify (a perf rewrite shaped by a live run, an unknown-root-cause debug, a step flagged "approach TBD pending discovery"). Also choose the judgment-grade tier when the plan meets `/sonnet`'s "must not be a Sonnet run" list (the last bullet of its *When NOT to use*), however procedural the edits look; check that list rather than keeping a copy here. The choice is made here, not punted downstream.
