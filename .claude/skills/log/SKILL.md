...
**Inputs.** `$ARGUMENTS` is `<type> [optional title or body]` — the first whitespace-separated token is the type; the rest is passed to the type-specific flow. The recognized types are listed once, in the Implementation; a sub-agent capture sentence spells out exactly that list, and nothing else restates it. If `$ARGUMENTS` is empty or the type isn't recognized, the skill lists the valid types and asks; it never guesses.
...
- **Misrouting a followup as a decision (or vice versa).** A "remember to do X next session" is a `followup`, not a `decision`; a "we chose X over Y because Z" is a `decision`, not a followup bullet. When the type token is absent and the body is genuinely ambiguous between the two, ask rather than guess — the two land in different files with different gates.
