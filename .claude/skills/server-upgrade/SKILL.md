...
8. **Removal vs rename — resolve it, don't hedge.** A `param-removed` / `field-removed` / `endpoint-removed` finding is the spec diff's view; **the diff cannot tell a genuine removal from a rename or a move.** Before deciding a fix direction for any removal you'd `file`, disambiguate: (a) scan the `<to>` spec for an *added* param/field/endpoint with the same role (a likely rename — the same report's `opportunity`/added entries are a hint); and (b) when it still matters, confirm against the upstream Jellyfin source (the controller/model on `jellyfin/jellyfin` — e.g. `gh api repos/jellyfin/jellyfin/contents/<path>`). A **rename/move** → map the new name behind version dispatch (the old behavior is preserved upstream). A **genuine removal** → the upstream *behavior* changed, so replicate it client-side or handle the loss gracefully. A verdict whose `draftIssueBody` says "confirm whether renamed or dropped" is **unfinished** — do that confirmation now; it's a one-line spec check plus, at most, one `gh api` read.
