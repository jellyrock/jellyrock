...
LABEL_TAIL='(newest first, dated; older ones move to [`DECISIONS.md`](DECISIONS.md)):**'
...
awk -v placeholder="$PLACEHOLDER" -v newplan="$work/plan.new" -v moved="$work/moved" \
  ...
  FILENAME == ARGV[1] && FNR == 1 && $0 == "---" { lfm = 1; next }
  FILENAME == ARGV[1] && lfm { if ($0 == "---") lfm = 0; next }
  FILENAME == ARGV[1] {
    if ($0 ~ /^- /) { flush_log(); li = $0 }
    else if (li != "" && $0 ~ /^[ \t]+[^ \t]/) li = li "\n" $0
    else flush_log()
