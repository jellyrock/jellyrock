...
| [`/server-upgrade`](server-upgrade/SKILL.md) `[<report-path>]` | opus | Triage a Jellyfin server release against JellyRock's API usage and file the issues for changes that actually affect us. Consumes the Phase-2 data report (`findings-candidates.json` from `npm run api-watch:findings`), investigates each candidate by reading its cited app-usage sites (resolving spec-contract-vs-runtime breaks, capability-guarded fallbacks, regeneration artifacts, enum switches, opportunity-worth-it, coverage symmetry), and emits a verdict per finding. The mechanical filer ([`scripts/server-upgrade.js`](../../scripts/server-upgrade.js), scaffold/plan/execute mirroring `/crash-report`) dedups against existing `[server-upgrade]` issues via a version-independent finding-key and does `gh issue create / comment / reopen` with labels. Human-gated — running execute is one batch approval per release (the graduated trust ratchet); nothing auto-files. RCs are tracked separately and don't generate issues. Per-finding deep-dive offloaded to `/issue-triage <N>` |
