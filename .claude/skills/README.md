...
| [`/crash-backtrace`](crash-backtrace/SKILL.md) `<N>` | sonnet | Attach a multi-frame backtrace + locals snapshot from Roku's analytics dashboard to a single already-filed crash issue (Roku exposes backtraces per-error, not in bulk). Accepts the backtrace via `@file` reference OR inline paste; reads the issue's cited version, builds (or reuses a cached) worktree, source-maps every frame, posts one enrichment comment. Worktree cache (~1h TTL at `/tmp/jellyrock-crash-wt-cache-<tag>`) makes consecutive enrichments on the same version near-instant. The per-issue follow-up to `/crash-report` |
