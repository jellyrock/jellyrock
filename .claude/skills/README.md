...
|---|---|---|---|
| [`/pr`](pr/SKILL.md) | sonnet | Create OR update a PR: a typed title (`fix:`, `feat:` … — `scripts/lib/pr-title.js` places it in CHANGELOG.md), category labels by judgment, a concise body that reads as the squash commit message, and one review-notes comment for reviewer-only detail. Runs the four-pillar judgment passes (tech-debt scan, decision-shape detect, followup capture). Detects existing open PR for the branch and routes to update-mode (diff title/labels/body/notes, ask, apply); aborts on merged/closed PRs. Judgment passes scope to since-last-render via a hidden marker in the notes comment on update. Checks title and body with `pr-body-check.js --body-file` before posting. Resolves every issue reference before posting (`pr-body-check.js --list-refs`) so another repo's issue is never a bare `#N`. Mechanical close-loop fires after merge via [`journal-sync.yml`](../../.github/workflows/journal-sync.yml). | Anytime you'd otherwise run `gh pr create` or `gh pr edit` directly — the ship moment for the four-pillar journal flow |
...
| [`/log`](log/SKILL.md) `<type>` | sonnet | Append/update a journal entry — routes `decision` / `followup` / `signal` / `running` to the right file. Diff-and-wait. `followup --replace=<substring>` revises one existing bullet in place instead of appending, so a multi-step entry can record that one step shipped. | After a decision-shaped commit, when deferring non-debt work, when adding a new upstream watch row, when changing what's in flight, or when one step of a multi-step followup lands |
| [`/done`](done/SKILL.md) `<slug-or-running>` | sonnet | Close a journal entry — followup → removed (recently-shipped line on `main`; on a branch, journal-sync writes it after merge), signal → completed status, or `running` cursor → recently-shipped | When a deferred followup ships, a watched signal resolves, or in-flight work completes |
...
| [`/crash-report`](crash-report/SKILL.md) `<csv-or-zip-path>` | opus | Turn Roku's weekly aggregate crash CSV (or zip) into tracked GH state via an enrich-before-file flow (`stage → enrich → file`). STAGE parses/filters + groups + builds the cited version in a temp worktree + source-maps `.brs:line → .bs:line` + dedups, writing a local worksheet. ENRICH folds in the dashboard backtraces you paste (7-day window, one click each), extracts each `&hNN`, and routes every crash to a disposition: `file` (scoped bug → new `[crash]` issue, born enriched), `aggregate` (architectural class — `&h29` big-library, `&h23` timeout, `&hec`+init race — upserted as one flat record per `file·function·line·version` onto the class epic), or `watch` (accepted noise, spike-only). FILE performs the writes. Dedup-aware + idempotent. Per-crash deep-dive offloaded to `/issue-triage <N>` |
| [`/crash-backtrace`](crash-backtrace/SKILL.md) | sonnet | **DEPRECATED** — superseded by `/crash-report`'s built-in `enrich` phase (enrich-before-file). Backtrace enrichment now happens inside `/crash-report` before filing, not as a per-issue follow-up after. Retained only as a redirect; `node scripts/crash-report.js enrich-issue --issue <N>` remains as a legacy escape hatch for a pre-migration already-filed issue |
| [`/server-upgrade`](server-upgrade/SKILL.md) `[<report-path>]` | opus | Triage a Jellyfin server release against JellyRock's API usage by editing the ONE per-version release-triage digest issue (auto-opened by CI) with verdicts and filing per-finding sub-issues for the changes worth standalone tracking. Consumes the Phase-2 data report (`findings-candidates.json` from `npm run api-watch:findings`), investigates each candidate by reading its cited app-usage sites (resolving spec-contract-vs-runtime breaks, capability-guarded fallbacks, regeneration artifacts, enum switches, opportunity-worth-it, coverage symmetry), and emits a verdict per finding. The mechanical filer ([`scripts/server-upgrade.js`](../../scripts/server-upgrade.js), scaffold/plan/execute mirroring `/crash-report`) dedups via a version-independent finding-key, files `file` verdicts as native GitHub sub-issues of the digest, inline-notes skip/monitor, rewrites the digest checklist, and closes it when fully triaged. Recurring floor findings self-resolve via the committed endpoint-availability registry. Human-gated — running execute is one batch approval per release (the graduated trust ratchet); nothing auto-files. **Pre-release path:** an RC, the literal `unstable`/`master`, or a datestamp as `<to>` triages proactively + locally (ephemeral fetch, re-diffable as it evolves) and writes a `.claude/handoffs/` note — files no GitHub issues. Per-finding deep-dive offloaded to `/issue-triage <N>` |
| [`/dep-major`](dep-major/SKILL.md) `<PR# \| package>` | opus | Walk a MAJOR dependency bump (incl. a Renovate GROUPED PR carrying several deps) through JellyRock's major-bump SOP so the human-review steps run the same way every time. Resolves each package + from→to range, fetches the upstream changelog for that range, classifies breaking changes + greps the call sites they actually hit, runs the mechanical gate that fits the dep's TYPE (runtime BS lib → `validate`/`build`/`lint:bs` + on-device `test:unit`/`test:rta`; JS/Node tooling → `test:scripts` + run the script; GitHub Actions → the PR's CI + a latent-path trace), and returns a go/no-go with any migration surfaced. Majors never automerge (Renovate org policy), so this is the review path. NOT for patch/minor (those automerge after soak) or Jellyfin server releases (use `/server-upgrade`) |
...
| [`/new-api-version`](new-api-version/SKILL.md) `X=<server-version>` | sonnet | [`docs/dev/jellyfin-server-versioning.md`](../../docs/dev/jellyfin-server-versioning.md) — stand up a new apiVersion tier (V3+); keeps `jellyfin-version-boundaries.yml` + `resolveApiVersion()` in lockstep. The follow-up when `/server-upgrade <rc>` shows a breaking shift |
