...
| [`/dep-major`](dep-major/SKILL.md) `<PR# \| package>` | opus | Walk a MAJOR dependency bump through JellyRock's major-bump SOP so the human-review steps run the same way every time. Resolves the package + from→to range, fetches the upstream changelog for that range, classifies breaking changes + greps the call sites they actually hit, runs the existing mechanical gate (`validate`/`build`/`lint:bs` + on-device `test:unit`/`test:rta`), and returns a go/no-go with any migration surfaced. Majors never automerge (Renovate org policy), so this is the review path. NOT for patch/minor (those automerge after soak) or Jellyfin server releases (use `/server-upgrade`) |
