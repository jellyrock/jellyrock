...
|---|---|---|---|
| [`/pr`](pr/SKILL.md) | sonnet | Create a PR using the JellyRock template + run the four-pillar judgment passes (tech-debt scan, decision-shape detect, followup capture). Mechanical close-loop fires after merge via [`journal-sync.yml`](../../.github/workflows/journal-sync.yml). | Anytime you'd otherwise run `gh pr create` directly — the ship moment for the four-pillar journal flow |
