...
**How to apply:** lead with intent-named options (`/release`, not `/run-tag-and-push`; `/format`, not `/run-prettier`). Mechanism-level names are fine for low-level helpers users don't invoke directly (e.g. a `worktree` dev script).
