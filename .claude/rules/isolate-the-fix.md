...
**Why:** "while we're here" tweaks add drift risk for no benefit. The example that named it: a proposal to relax a working cron schedule when the *script*, not the *schedule*, was broken. Cosmetic dislike (e.g. "this path looks ugly") is not a reason to refactor a working upstream convention either.
