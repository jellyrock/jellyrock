...
**Where the proof goes.** "Record it" does **not** mean "write it into the code comment you happen to be typing." The repro, the measurement, the before-state, the numbers you took to prove the point — those go in the **commit message** or the **journal entry** (`docs/progress.md`, `tech-debt.md`, a decision record), where `git blame` and `/catchup` already reach them. The comment gets only what a reader needs to *change this code correctly*: the rule, and the non-obvious why behind it. Writing the evidence in both places isn't thoroughness — it's two copies that will drift, and the comment is the one nobody updates.
