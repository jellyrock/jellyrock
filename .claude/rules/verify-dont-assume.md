...
**Why:** this keeps biting. A third-party app's settings menu and a cloud service's API response shape have both been asserted from training knowledge and been wrong — confidently, across several turns, until the user shared the actual output. The failure is asserting unverified external behavior as fact *before* any ground truth exists, not "ignoring a screenshot."
...
- If you can check the live system yourself — run the actual command, make an API call, fetch the real docs page — do that and cite it. Fetching/verifying beats recalling.
