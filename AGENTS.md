...
- **A new git worktree has no git hooks until `npm ci` runs inside it.** Git reads hooks from each worktree's own gitignored `.husky/_/`, so a bare worktree commits and pushes with no checks and no warning. Put it under `.claude/worktrees/<name>`, never a Claude Code session scratchpad: that path contains a UUID, npm masks UUIDs in its output, and `npm ci` then fails. Run `npm ci` in the worktree and confirm `.husky/_/pre-push` exists before committing. Don't symlink `node_modules` instead: `.gitignore` doesn't match the link, and the branch would run the main checkout's dependencies. In Claude Code, creating one (`git worktree add`, an `Agent` with `isolation: "worktree"`, `EnterWorktree`) prompts the user via `permissions.ask` in [`.claude/settings.json`](.claude/settings.json), so say why before asking. **A hook in a worktree runs with `GIT_DIR` set**, which git obeys over any `cwd`, so anything the hook spawns that runs git elsewhere hits the real repo: pushing from a worktree once moved the pushed branch to a test commit and wrote `core.bare=true` into the shared config. `.husky/pre-push` unsets those variables and the script-test helpers scrub their own env ([`tests/scripts/unit/_helpers/git-env.js`](tests/scripts/unit/_helpers/git-env.js)); keep both if you add a hook step that shells out to git
...
**Cross-cutting authoring norms** — the working principles that apply to *any* change, regardless of subsystem — live in [`.claude/rules/`](.claude/rules/): `cost-efficiency`, `derive-dont-duplicate`, `dogfood-changes`, `intent-based-naming`, `isolate-the-fix`, `iterate-on-evidence`, `prove-dont-dismiss`, `reuse-existing-tooling`, `verify-dont-assume`. They are **load-bearing** and auto-load into Claude Code every session; read them before any non-trivial change. (Agents or tools that don't auto-load `.claude/rules/` should read them from that directory — the filenames say what each covers.)
