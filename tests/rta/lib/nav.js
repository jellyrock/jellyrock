 ...
 * group gains focus, so teleporting focus onto a child gets reverted. Enter via
 * the GROUP (it focuses its current index) and walk with real presses.
 *
...
export async function navSubtitlePanel(ctx) {
  ...
  // The panel slides in over 0.4s (panelAnime) and then auto-searches, so the
  // wait has to outlast a provider round trip through the server, not just the
  // animation. Visibility is the open signal: the node exists while closed.
  await waitFor('#subtitlePanel.visible', (v) => v === true, {
    label: 'subtitle panel open',
    timeout: 15000,
  });
  // Settle past the slide before anything reads or photographs the panel — the
  // north-star rule: a screen that has begun rendering has not finished.
  await sleep(1500);
}
