...
- `lib/` — `driver` (env + deploy + relaunch), `steps` (press/getVal/waitFor/waitFocused), `seed` (registry seeds), `registry` (whole-registry snapshot/verified restore), `jellyfin` (demo REST), `nav` (per-screen navigation), `diagnostics` (failure-time device-state dump + the run's failure record).
...
- **A timeout must report what it SAW, so throw via `diagnosedError`, never a bare `new Error`.** `lib/diagnostics.js` attaches the state the device was actually in (active view + `loadState`, focused node, row counts, seeded server/user identity) and appends a record to `out/rta/failures.jsonl` that `scripts/rta-run.js` folds into `run-meta.json`. The capture runs only at the throw site, after a poll loop has given up — never inside a tick — so it costs nothing on the success path (measured: 26 ms at the boundary). Prefer passing state the loop **already read** as `observed` over re-reading it; that is what turns `2 row(s) present` into `rowTypes=[Chapter, Person]`, i.e. the difference between *late* and *absent*. Never dump a whole node into a record — `JellyfinUser` carries `authToken`; read identity by named field. Full shape in [`docs/dev/rta-tests.md`](../../docs/dev/rta-tests.md#when-a-wait-times-out-it-reports-what-it-saw).
