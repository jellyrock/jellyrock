...
|---|---|---|
| `FN` Function `keyPath` | 15 | ODC observes a **field**. `getChildCount()` / `subtype()` are calls, not fields, so the primitive cannot apply at all. |
...
| `SETTLE` Plain field settle | 50 | The primitive could apply; it is ruled out below. |
