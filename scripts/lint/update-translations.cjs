...
function checkPluralCompleteness(keys) {
  ...
  for (const base of baseKeys) {
    ...
    if (missing.length > 0) {
      errors.push(
        `Incomplete plural set "${base}": missing ${missing.map((s) => base + s).join(', ')}`,
      );
    }
  }
...
async function main() {
  ...
  if (hardcodedViolations.length > 0) {
    errors.push(
      `${hardcodedViolations.length} hardcoded translation key(s) (use translationKeys.* constants)`,
    );
  ...
  for (const filePath of nonEnLocales) {
    ...
    for (const key of localeKeys) {
      ...
      if (JSON.stringify(enP) !== JSON.stringify(locP)) {
        errors.push(
          `${fileName}: placeholder mismatch for "${key}" (en_US: ${enP.join(',') || 'none'} vs ${locP.join(',') || 'none'})`,
        );
      }
    }
  ...
  if (FIX_MODE && missingLangs.length > 0) {
    for (const code of missingLangs) {
      ...
      if (meta) {
        ...
        console.log(
          c(`  Warning: no metadata for "${code}" \u2014 using code as placeholder name`, 'yellow'),
        );
    ...
    errors.push(
      `${missingLangs.length} locale file(s) missing from languages.json: ${missingLangs.join(', ')}`,
    );
