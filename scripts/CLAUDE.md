...
- **Icon PNGs are generated, not hand-authored.** SVG sources live in
  ...
  pre-push (auto-fix when `resources/icons/*`, the build script, or
  [`scripts/lib/png-compare.js`](lib/png-compare.js) changes) plus
  `npm run icons:check` as a check step, and in CI by the repo drift gate in
  `tests/scripts/unit/generate/icons-build.test.js`.
- **Asset drift is measured in decoded PIXELS, not bytes**
  ([`scripts/lib/png-compare.js`](lib/png-compare.js), shared by the icon and
  gradient generators). sharp bundles its own libvips + zlib, so the same image
  re-encodes to different bytes when sharp moves — comparing bytes had
  `icons:check` failing on 55 pixel-identical files for three months. Two
  consequences worth knowing before you touch either generator: a build leaves a
  pixel-identical PNG alone (use `--force` to deliberately re-encode), and a
  bloated-but-pixel-identical PNG is not detected.
