...
export function generateIndex() {
  ...
  const captured = fs
    ...
    .map((d) => d.name)
    .filter((name) => fs.readdirSync(path.join(shotsDir, name)).some((f) => f.endsWith('.webp')))
  ...
  const hasShot = (name) =>
    galleryLang && fs.existsSync(path.join(shotsDir, galleryLang, `${name}.webp`));
  ...
  const cell = (name) =>
    `<a href="${galleryLang}/${name}.webp"><img src="${galleryLang}/${name}.webp" width="260" alt="${humanize(name)}"></a><br>${humanize(name)}`;
  ...
  md += `Every screen in the app, captured on a real Roku — the full gallery below is in `;
  md += `**${nativeName(galleryLang)}**.`;
  if (captured.some((c) => c !== galleryLang)) {
    md +=
      ' The other languages are the **Roku Channel Store** listing assets, so those folders' +
      ' hold only the 6 store screenshots — not the full gallery.';
  }
  md += '\n\n';
