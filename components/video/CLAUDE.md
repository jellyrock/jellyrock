...
- **Nothing may touch `VideoPlayerView` after its `host.callFunc(...)` into `PlayerHostView` returns.** The host can destroy the calling player synchronously — a remount runs its `onDestroy`, which releases its members — and control then returns into a component that no longer has them. Measured on a Stick 4K: `bufferCheck` touching `m.bufferCheckTimer` after `onLiveStreamStalled` restarted the channel crashed the app (`&he4`). `switchToQueueItem`, `exitFromPlaybackError` and `bufferCheck` end on their host call for this reason, and `handleTransport`'s voice `stop` returns only a literal after its `exitPlayback`.
...
- **A stream that never reported `start` still reports its end, marked `Failed`.** Don't skip that stop: it is what makes the server stop the transcode. For Live TV, **exactly one release per stream**: a stream that reported `start` is released by its stop, and one that never did by a close by id, with its stop leaving `LiveStreamId` out (`endClosesLiveStream()` decides, `liveStreamReportFields()` builds the report). Never both: what a stop naming a never-started stream does varies by server version and by who else is watching, and a second release ends another viewer's shared stream (table in playback.md). And don't send it unmarked: the position read before the first frame (usually 0) would replace the resume point (#969). [playback.md](../../docs/architecture/playback.md#reportplayback--server-side-reporting) has the full rule.
- **Never just `STOP` the video load (`m.LoadMetaDataTask`) when the view goes away — `releaseVideoLoad()` decides.** A load stopped while its `PlaybackInfo` is opening a Live TV stream loses the only answer that names the stream, and the server keeps it open; a load that already holds the stream must have it closed. The load records its stage on `liveStream`, and the view marks it `handedOver` or `released` when it takes or closes the stream, so exactly one party releases each stream ([playback.md](../../docs/architecture/playback.md#a-live-stream-the-player-never-held), `source/utils/liveStreamHandoff.bs`).
