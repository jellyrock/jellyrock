...
- **A `host.callFunc(...)` from `VideoPlayerView` into `PlayerHostView` must be the caller's last statement.** The host can destroy the calling player synchronously — a remount runs its `onDestroy`, which releases its members — and control then returns into a component that no longer has them. Measured on a Stick 4K: `bufferCheck` touching `m.bufferCheckTimer` after `onLiveStreamStalled` restarted the channel crashed the app (`&he4`). `switchToQueueItem`, `exitFromPlaybackError` and `bufferCheck` all end on their host call for this reason.
