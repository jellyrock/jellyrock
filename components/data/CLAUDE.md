...
- Don't add behaviour to `ContentNode` subclasses unless it's transformer/sync logic that genuinely belongs with the data shape.
- Don't omit `value="…"` for typed fields by accident — Roku's auto-coercion of empty values can surprise.
- Don't store values in registry whose typed shape isn't reflected here. The XML schema is what BSC validates against; an undocumented field is an untyped field.
