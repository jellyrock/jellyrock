...
- **A routed screen owns its own load spinner.** If a screen fetches data on open, it `startLoadingSpinner()`s when the fetch begins and `stopLoadingSpinner()`s when data arrives — callers just navigate. Don't stop a spinner synchronously right after a `navigateTo` to a *different* view: the nav is async (settles at `NavigationEnd`) so `activeRoutedView` is still the outgoing view, and stopping re-shows it for a frame. See [navigation.md → Loading spinners across navigation](../docs/architecture/navigation.md). **Name the wait by its kind**: a screen waiting on the server for its own data opens a named wait on itself — `screenWaits.begin(m.top, "results")`, ended on every exit path with `screenWaits.finish(m.top, "results")` — and the scene shows it only while that screen is active, so it cannot outlive the screen, be ended by another, or vanish when the viewer comes back to it. `startLoadingSpinner()` is the plain spinner for call sites not yet given a kind. The kinds and their rules for Back and text are in [navigation.md → What kind of wait a spinner is](../docs/architecture/navigation.md#what-kind-of-wait-a-spinner-is).
...
- **You are ON the render thread here, so `m.global` and Task nodes are FREE to touch — the price is paid by Task-thread code, not by you.** Ownership decides it: nodes are render-owned by default, so an `m.global` field read costs **2.0 µs from a component and 93 µs from a Task** (Stick 4K, ~46×). ⚠️ **That does not make loops free.** Removing the rendezvous took a per-entry walk over Task nodes from 132.6 µs to **20.1 µs per entry** — 6.6×, not 46× — because the rest is plain interpreter work. Budget the loop as well as the crossing; the measured table is in [threading.md](../docs/architecture/threading.md#measured-findings).
...
- **Never launch one Task per item.** A `launchTask()` inside a `for` / `for each` / `while` is a **build error** — the `no-task-fanout` plugin — unless its argument is a fixed `m.<field>` slot the loop doesn't rebind. One thread per library is the shape #728 actually took, and correct teardown does not save you: the crash is concurrent launches inside a single screen load, not threads leaked across navigation. The sanctioned fix is one orchestrator Task servicing every item over [`apiPipeline`](../source/api/apiPipeline.bs) — [`LoadLatestRowsTask`](home/LoadLatestRowsTask.bs) is the reference. Parking a *fresh* node in an `m.` slot each turn is the same fan-out wearing a stable name, and is flagged too. So is a loop calling a same-file helper that launches a new node per call: if the helper keeps at most one run live however often it is called (a guard, or a cancel first), state that on its launch line — `' bsc-disable-next-line no-task-fanout <the bound>` (`HomeRows.startResumeLoad` is the model).
- **An invalid node is a silent no-op, not a crash.** A raw `m.someTask.control = "RUN"` faulted when `m.someTask` was invalid; `launchTask` returns `false` instead. Better for users (a row that doesn't load beats an app that dies), worse for diagnosis — so debug builds print `[TASKS] launchTask() called with an invalid node`. Check the return only where you have something useful to do with a failed launch — typically a "loading" flag that only the run's delivery clears, which must not be left set by a launch that never started (`HomeRows`' section loads, `m.isLoadingResume = launchTask(…)`).
- **Past the thread watermark a launch WAITS and `launchTask` returns `true`** ([ADR 0041](../docs/adr/0041-task-launch-queue.md)): the node sits in `m.global.taskLaunchQueue` reading `state = "init"` until a slot frees. So `false` means only an invalid node or a full queue. **To ask "is my run still going?", call `isTaskInFlight(node)`, not `node.state = "run"`**, which calls a waiting run finished; replacing it then sends the new one to the back of the queue (`PlayerHostView.onPlaybackInfoRefreshDue`). A node STOPped while it waits is never started.
...
- **A run that can be restarted mid-flight needs a NEW Task node, not a relaunch.** Measured on device: `STOP` then `launchTask` on the same node in one callback, while its function is still running, is a race — sometimes the launch is silently ignored (nothing starts, nothing errors) and sometimes it starts, so neither outcome can be relied on. A new node launched in that callback always starts, and the stopped function makes no further progress, so no threads stack up. **Paging from the Task's own delivery handler is the same trap without a STOP:** inside the observer for the field the Task writes last, the node still reads `run`, and relaunching it there was dropped 6 of 6 times (the TV guide's channel pages — [threading.md](../docs/architecture/threading.md#measured-findings)), so each page gets a new node too. **So is keeping one node for a component's whole life and launching it again from a later callback** (write the input, `launchTask`, no STOP): if the previous run has not returned, the launch is ignored every time (40 of 40), nothing is queued, and a Task that re-reads its input after its request can then deliver a result mixing the two (`MoviePresenter.loadLogo` did — [threading.md](../docs/architecture/threading.md#measured-findings)). Only a site that waits for `state = "stop"` before relaunching is safe that way (`TrickplayCarousel`, which says so on a suppression). **Use `m.task = replaceTask(m.task, "<Type>", "<observedField>", { inputs })`** (`source/utils/tasks.bs`), and give the handler its event with `isCurrentTaskEvent(event, m.task)` as its FIRST statement — Roku does not promise that `unobserveField` cancels an event already queued, so without it a replaced run's event reads the new node's empty output and unobserves it. First means before a `screenLoad.resolve` or a spinner stop too. **The guard only rejects a REPLACED run's event, so where the context changes (a new song, a new item) and the new context may start no run of its own, release the old run there:** otherwise it is still the current node and its late delivery passes the guard. `AudioPlayerView.pageContentChanged` does this on every track change, because a fully tagged song launches no metadata fetch (decision `audio-metadata-release-on-track-change`); the build gate cannot see this case. `SearchResults.searchMedias` / `loadResults` is the reference; the evidence is a row in [threading.md](../docs/architecture/threading.md#measured-findings), pinned by `tests/source/unit/platform/TaskRelaunch.spec.bs`. **A STOP then `launchTask` of the same node in one function (or after a same-file helper, `releaseTask` or `replaceTask` that STOPs it, with the result not assigned back) is a build error** — the `no-same-node-relaunch` plugin. **So is a `launchTask(m.x)` with no new node assigned to `m.x` earlier in the function, and a handler of a `replaceTask` node, in the same file, that does not open with `isCurrentTaskEvent`** (logging aside). A launch that provably cannot overlap a running one is suppressed on the line with its reason. The sites that predate the rule are listed in that plugin's `PENDING_MIGRATIONS` and are being audited and migrated; delete a site's entry when you do (the build tells you to).
...
- **A key event can arrive AFTER your `onDestroy`, and `m.isDestroyed` is already handled for you.** sgRouter leaves a closing view mounted, visible and in the focus chain until the removal it defers to a later message-loop turn — so a screen whose `onDestroy` has already released its node references can still be handed one more key press, and every bare `m.someNode.…` in `onKeyEvent` is an `&hec` waiting to fire. The [`auto-destroyed-guard`](../docs/architecture/build-and-tooling.md#scriptsbsc-pluginsauto-destroyed-guardcjs) plugin injects the flag and the early return at transpile time for any component declaring both `onDestroy` and `onKeyEvent`, so **don't hand-roll it** — and don't rely on `isValid(m.node.field)`, which guards the field, not the receiver.

- **In `onDestroy`, detach EVERY observer before releasing ANY reference.** The window is the mirror image of the one above: a writer can deliver an observed field one last time while your `onDestroy` is still running, and if you have already nulled a reference that handler dereferences, it throws `&hec`. That writer is not only an EXTERNAL one (a Task thread, the native playback engine) — **a component's own in-flight async continuation counts too**: a `fetchAsync().then()` or a task observer belonging to this very component resolves after teardown starts, which is what `ItemDetails.itemContent` turned out to be. That is what #733 was. Two phases, in this order: all the `unobserveField`s, then all the `= invalid`s. [`unobserve-before-release`](../docs/architecture/build-and-tooling.md#convention-plugins) warns when you interleave them in a way that can actually bite — **reorder** to clear it rather than adding an early return to the handler, because some observers legitimately run during teardown (stop-reporting, dialog cleanup) and a guard would silently skip those too.

- **Wire an interface field to its handler ONCE, and let `onDestroy` decide which form.** Declaring both an XML `onChange` and an `m.top.observeField()` registers two observers and runs the handler twice on every write — silent when the handler is idempotent: `OSD.itemData` carried it for six months and `IconButton.isButtonSelected` for five, and the OSD one was only caught because a test counted 4 appended buttons where it expected 2. Which form is not a style pick, and it is not a judgment call either — one structural question decides it:
  - **No `onDestroy` in the codebehind → XML `onChange`.** The component releases nothing, so no handler of its can dereference a nulled reference, and the observer dies with the node. This is the majority: 42 of the 53 components using `onChange` are this case and correctly need no teardown at all.
  - **An `onDestroy` in the codebehind → `m.top.observeField()` in `init()`**, with a matching `unobserveField()` above every release in `onDestroy`. An `onDestroy` is the declaration that this component releases state; from that moment any handler that can still run is a hazard, and an XML `onChange` is the one form that cannot be detached.
  ...
  ⚠️ **Why the rule keys on `onDestroy` rather than on "can a writer of this field outlive teardown?"** The second is the truer statement of the risk — but it is undecidable by a linter, so it leaves every site resting on somebody's memory, which is how this class survived twice already. The blunt structural question is provable, mechanically checkable, and conservative in the safe direction. The cost is a real edge: adding an `onDestroy` for an unrelated reason (stopping a task, say) now also requires converting that component's `onChange` fields. That lands at exactly the moment the hazard becomes real.

  All three halves of this are build **errors** — [`field-observer-wiring`](../docs/architecture/build-and-tooling.md#convention-plugins) fails the build on an `onChange` in a component that tears down (`undetachable-observer`), on a field wired twice (`duplicate-field-observer`), and on an `unobserveField` against a field only an XML `onChange` registers (`ineffective-unobserve`). You should not be able to reintroduce any of them by hand.
...
- **Always go through `source/utils/dialogs.bs`** — `showAlertDialog`, `showConfirmDialog`, `showChoiceDialog`, `showListDialog`, `showInfoDialog`, `showKeyboardDialog`, `showQuickConnectDialog`. The helpers are the only path that stamps the shared overlay id; anything that skips them is invisible to `isOverlayDialogOpen` / `isDialogOpen` / `cancelOpenDialog`. No hand-rolled `appendChild` overlay is left, and a BSC plugin now makes adding one a build error. Canonical call sites: `ItemDetails.onWatchedButtonPressed` / `onDeleteButtonPressed`.
- **Never re-derive chrome or layout.** Chrome comes from `JRDialogPanel`, geometry from `source/utils/dialogLayout.bs` (pure, unit-tested). A dialog supplies a body and a footer, nothing else. Three private copies of this is how the app ended up with two dialog looks that every gate passed.
- **The result arrives on the dialog node's own `result` field**, shape `{ cancelled, confirmed, buttonIndex, buttonText, optionIndex, value, externallyCancelled }`. **`externallyCancelled` is the key to read if your handler ACTS** — it is true when code closed the dialog (`cancelOpenDialog`, or `presentOverlayDialog` superseding you) rather than the user, and `cancelled` cannot tell you that on its own. **Overlay dialogs only** — the keyboard dialog resolves through Roku's modal `close`, which cannot tell the two apart, so it reports `false` even when `cancelOpenDialog` closed it. Don't build an acting handler on a keyboard dialog. From a component, pass `onResult` (a function name in *your* scope) and the helper wires the scoped observer. From **main-thread code** (`source/*.bs` in `Main()`'s scope) there is no component scope: omit `onResult`, observe with `dialog.observeField("result", m.port)`, and compare node identity (`node.isSameNode(m.myDialog)`) before acting — `replayRoute.onServerSwitchDialogResult` is the reference.
- **Set every text field BEFORE presenting, never after.** `renderTracking` fires once on none→full, so a dialog never re-lays-out after mount and later text draws on top of the old layout.
- **Tearing one down: two verbs, not interchangeable.** `abandonDialog(dialog)` delivers NOTHING — right in your `onDestroy`, where the scope that would receive the result is going away. `cancelOpenDialog()` delivers a **cancelled** result — right when you need the screen clear but someone else owns the dialog and is holding state until it answers. Wrong way round strands a flow with no symptom at the call site. **And if your result handler ACTS — navigates, mutates state — the ORDER matters too**: abandon yours before anything else can cancel it, because cancel is indistinguishable from the user pressing Back and will fire your action from inside someone else's flow. Rule and the test to apply it: [`dialogs.md`](../docs/architecture/dialogs.md#presenting-and-tearing-down).
- **Overlay dialogs are appended to the scene**, not to your screen, so they survive your `onDestroy`. A screen that opens one owns tearing it down. Exactly ONE is on screen at a time — `presentOverlayDialog` supersedes the incumbent, so you need no re-entrancy guard at your call site.
- **Key behavior belongs in `source/utils/dialogKeys.bs`**, not in a component's `onKeyEvent` — it is pure, so a test can reach it. `listDialogKeyAction` and `buttonDialogKeyAction` are the two models.
- **`SceneManager` has no dialog methods left.** `isDialogOpen` is the one survivor, and it is a *query*.
