...
- **Key behaviour belongs in `source/utils/dialogKeys.bs`**, not in a component's `onKeyEvent` — it is pure, so a test can reach it. `listDialogKeyAction` and `buttonDialogKeyAction` are the two models.
