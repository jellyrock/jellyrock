...
- **A run that can be restarted mid-flight needs a NEW Task node, not a relaunch.** Measured on device: `STOP` then `launchTask` on the same node in one callback, while its function is still running, is a race — sometimes the launch is silently ignored (nothing starts, nothing errors) and sometimes it starts, so neither outcome can be relied on. A new node launched in that callback always starts, and the stopped function makes no further progress, so no threads stack up. `ExtrasRowList.startRun` is the reference; the evidence is a row in [threading.md](../docs/architecture/threading.md#measured-findings), pinned by `tests/source/unit/platform/TaskRelaunch.spec.bs`.
