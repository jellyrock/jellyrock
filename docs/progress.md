...
- **Cast transport verbs no-op on audio playback** (remote control #667 follow-up): a `Playstate` `Pause`/`Unpause` cast to a JellyRock session playing **audio** is *delivered* over the long-poll (verified on `BATCAVE`: 204, no dispatch error in the console) but doesn't affect playback (`IsPaused` stays false). `Play` works over the identical channel, so this is **not** the #667 transport — it's the shared main-thread `remoteDispatch.dispatchTransport` active-view path (subtypes `PlayerHostView`/`VideoPlayerView`/`AudioPlayerView`) or the audio player's `handleTransport`, both shared with the `ws://` path (#666) + voice — so likely pre-existing for audio cast. Re-verify on video with a clean direct-playing item; if audio-specific, fix in the audio player view / active-view detection. Epic #669.
