...
- **Live TV channel that finished** → `restartLiveChannel()` (restart the same channel, host-internal remount). A live feed that drops mid-stream surfaces as `finished`, which is why the restart exists. A stream that ends without ever playing (an on-demand playlist listed as a channel, joined at its end) surfaces the same way, so the restart is budgeted: an end after 30 s of progress (how far into the stream the mount got from where it started playing, so paused and buffering time, and the player's replay of a stale live window, don't count) resets the count, any other end adds one, and past three the player shows the playback error instead. Only the restart keeps the count; every other mount (first play, channel switch, queue advance) starts a fresh one. The host just forwards mount, state and end events to `liveRestartOnMount()` / `liveRestartOnState()` / `liveRestartOnEnd()` in `source/utils/liveTv.bs`, which hold the whole rule
- **Live TV stream that stalled** (not an end): `VideoPlayerView.bufferCheck` sees no buffering progress across a 30 s tick (`classifyBufferCheck()` in `source/utils/bufferStall.bs`) and calls the host's `onLiveStreamStalled()`, which clears the screen the same way and asks `liveRestartOnStall()`: a mount that had made 30 s of progress restarts (a dropped feed), any other stall shows the error "This channel's stream stopped" and stops the stream. No restart budget for stalls, because each one is only detected 30-60 s into the spinner. A movie or episode that stalls shows the error directly, as before
