...
- **`stop` also means "finished".** A Task that returns normally reports `stop` too, but only after its answers were delivered, and delivery ends the watch (`forgetCaller()`), so it matches no slot (`apiPoolSlotsWatchedBy()`).
...
- **So the watch is one per Task, not one per request.** An `apiPipeline` run can have several long requests of one Task on slots at once, and dropping the subscription when the first is answered would leave the rest with no watch. `watchCaller()` subscribes only for a Task no other in-flight request is watching, `forgetCaller()` unsubscribes only when none is left, and when a Task stops, every slot it was waiting on is stopped. A pipeline request that asks for a long limit is still bounded by its run's budget (`apiPipelineBegin`'s `budgetMs`), so it needs a larger budget as well.
- **`roku-requests` ends a stopped request through its timeout path.** Its check for a quit reads `m.cancel_and_return` rather than its local `cancel_and_return`, so a stop cancels the transfer, logs `[http] Event Timed Out`, and would retry if the request allowed retries. `executeHttpRequest()` passes no `retryCount`, which is what keeps a stop to one cancel; see the note in [`ApiTask.xml`](../../components/api/ApiTask.xml).

Only reads, for the same reason as skipping. The Live TV `PlaybackInfo` that opens a live stream asks for a long limit too (`timeouts.LIVE_OPEN_MS`), and it is exactly the kind of request that must not be stopped: the server opens the stream anyway, and only the answer names the stream the app has to close ([playback.md](playback.md#a-live-stream-the-player-never-held)). A render-thread caller has no waiting Task to watch, so a long request made from the render thread would not be stopped; none exists today. The rules are pinned in [`apiPoolStop.spec.bs`](../../tests/source/unit/api/apiPoolStop.spec.bs), and the behavior end to end in the `slow-library` RTA spec.
...
**So `ApiTask` does not declare `quit`; the field exists only while a stop is pending.** Reading a field the node has costs more than reading one it lacks. Measured 2026-09-25 on the 512 MB Stick against a local 10.11 server, 4 alternating rounds of 5 Home launches, 300 requests per arm: a median of 6.9 ms per read with the field declared against 3.7 ms without, and more total wait in all 4 rounds (+3% to +28%). Declaring it would tax every request in the app for the sake of the rare one that is stopped, so `apiSlotQuit()` adds it and `apiSlotClearQuit()` removes it, and a unit test pins that `ApiTask` declares no `quit`.
