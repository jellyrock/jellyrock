...
| `control = "STOP"` on a Task that is NOT parked: its `wait()` has just been answered but its thread has not run yet (the answer written and the STOP made in one render-thread callback), or it is running ordinary code between waits | Render | **It makes no further field write, and does not wake to run on.** 0 of 20 answered-then-stopped Tasks ran their next write, 0 of 10 stopped mid-way through a loop of writes wrote again, 0 of 10 stopped during local work made the write that followed, on each device. A field READ after the STOP did not return either (0 of 10; the same Task left running printed its line, the control). So a stopped Task can neither send a request nor submit a close after the STOP, which is what lets `VideoPlayerView.releaseVideoLoad` close a live stream and stop its load without the load closing it again ([playback.md](playback.md#a-live-stream-the-player-never-held)). Gated in every run by [`TaskStopProgress.spec.bs`](../../tests/source/unit/platform/TaskStopProgress.spec.bs) | Streaming Stick 4K and 512 MB Streaming Stick `3600X`, Roku OS 15.3.4, 2026-09-26 |
