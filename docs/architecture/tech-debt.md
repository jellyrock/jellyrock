...
#### `sgrouter-view-unconditional-promise-dep`

- **area**: `components/JRGroup.bs` (extends `sgrouter_View`), the vendored `sgrouter_View.xml`
- **issue**: ADR 0021 removed JellyRock's own `promises.brs` import from the `JRGroup`/`JRScreen` `onDestroy` floor, but `JRGroup` still inherits `promises.brs` — plus `router.brs`, `RouterState.brs`, `rodash.brs`, `interfaces.brs`, `bslib.brs` — into its script scope unconditionally via `sgrouter_View.xml` itself, a dependency of the router library, not JellyRock code. Whether this residual exposure alone is sufficient to reproduce issue #689 (Roku OS 12.5, "Error parsing multiple XML components" install failure) is untested; no OS 12.5 device is currently available to verify.
- **direction**: If OS 12.5 hardware becomes available, test whether #689 reproduces with ADR 0021 applied but no other change. If it does, decoupling `JRGroup` from `sgrouter_View`'s bundled dependencies (or getting the dependency made conditional upstream in sgRouter) needs its own ADR — not a fix on JellyRock's side alone.
