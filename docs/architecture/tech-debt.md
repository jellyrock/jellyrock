...
#### `unify-grid-item-components`

- **area**: [`components/ItemGrid/GridItem.bs`](../../components/ItemGrid/GridItem.bs) + [`.xml`](../../components/ItemGrid/GridItem.xml), [`components/ItemGrid/GridItemSmall.bs`](../../components/ItemGrid/GridItemSmall.bs) + [`.xml`](../../components/ItemGrid/GridItemSmall.xml), [`components/ItemGrid/BaseGridView.xml`](../../components/ItemGrid/BaseGridView.xml) (the two `MarkupGrid` declarations at lines 45 / 57 that pick between them).
- **issue**: `GridItem` (264×396 default, reshapes to 323×323 for music items via `setSquarePosterLayout`) and `GridItemSmall` (230×320) are shape-duplicates after the `JRPlaceholder` migration. Identical child structure (`JRPlaceholder` + `JRPoster` + `itemIcon` corner-badge + external title label), identical `onPosterLoadStatusChanged` state machine, identical content-type → poster URL routing. Real differences: cell dimensions, external-title field name (`itemText` vs `title`), grid-titles field source (`gridTitles` vs `showItemTitles`), and texture-management ownership (`GridItem` unloads off-screen textures via `unloadGridTexture` / `forceUnloadGridTexture` / `evaluateGridTextureState`; `GridItemSmall` doesn't). Two near-identical components means every per-feature change touches both: the placeholder migration itself did so, the eager-empty-`uri` fix did so, the `resolveItemPlaceholderType` wiring did so. The duplication compounds.
- **direction**: Unify into a single `GridItem` component. Parent-passed `width` / `height` fields drive sizing. Harmonize field names (`gridTitles` everywhere). Extend texture management to all cells (`GridItemSmall` gains memory savings on long scrolls). `BaseGridView`'s two `MarkupGrid` declarations collapse to one. The `Genre` action-tile branch keeps its dedicated `genreBackdrop` + `genreLabel` nodes; the main media branch keeps the `JRPlaceholder`. Cleanest approach: extract the shared logic into a new `JRGridItem` base or — simpler — promote `GridItem` to handle both sizes by accepting `cellWidth` / `cellHeight` fields and deleting `GridItemSmall.xml/.bs` entirely.
