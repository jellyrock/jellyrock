...
- **issue**: Top-level `scripts/*.js` are ESM (5: `catchup-state.js`, `changelog-syncer.js`, `crash-report.js`, `journal-sync.js`, `run-roku-tests.js`); the other 4 top-level scripts are `.cjs`. `scripts/lib/` is all `.cjs`; `scripts/lint/` is all `.cjs` except `floor-coverage-check.js` (ESM by necessity — it imports named exports from the ESM `findings-candidates.js`, which a `.cjs` file can't `require()`). `package.json` has `"type": "module"`, so ESM is the modern default for new code. Forward rule for net-new scripts is documented (ESM `.js` unless `require()`'d), but the existing `.cjs` files in `lint/` and `lib/` are kept for now.
