...
- **direction**: (1) Audit the persistent-task floor and keep it minimal — the pool is intentionally capped at 3 (`source/api/CLAUDE.md`: don't grow without measuring). (2) Bound remaining concurrent *transient* tasks. The `HomeRows` latest-media rows are done, and a sweep of every `CreateObject("roSGNode", "*Task")` site under `components/` + `source/` (2026-08-02) found **no remaining per-item Task fan-out** — no launch site currently scales with library size. This corrects an earlier claim here that the large-library grid was the next fan-out: `ItemGrid/LoadItemsTask2` is a *single* Task node making sequential blocking `fetchJson` calls, so its problem is latency on one thread, which `source/api/apiPipeline.bs` addresses and a thread bound would not. **Confirmed by measurement 2026-08-04** (n=4, Streaming Stick 4K, one movie library, 8 genres): the genre loop splits **78% wait / 22% emit** — the inverse of Home's 51%-emit profile — so `apiPipeline` is the mechanism and the orchestration job pool would attack the wrong 22% while leaving the 757 ms of serial fetching untouched. Mechanism is therefore chosen **per orchestrator from a measured split**, never carried across; see [`docs/dev/home-first-paint-performance.md`](../dev/home-first-paint-performance.md). What remains is the per-screen floor (each navigation spawns its own task nodes), not any one hot path. (3) Enforce that every routed screen's `onDestroy` releases its Task nodes (the `jrscreen-on-destroy` / `observe-without-on-destroy` BSC plugins already flag missing teardown — extend the discipline to explicit Task-node release). Note: the #744 `SideEffectTask` FIFO conversion is **task-count-neutral** — it stays a single Task; the per-request `ApiResultNode` children are plain Nodes (`extends="Node"`), not Task threads.
