...
- **direction**: Collapse to a single promise model once BrighterScript **async/await** ships — `await fetchAsync(...)` restores the linear/branching readability that today justifies keeping blocking `fetchRes` in Task orchestrators. Re-open decision `promise-native-interface-fetchres-exception` at that point and, in the same pass: **(a)** convert kept-Task internals (`home/LoadItemsTask`, `components/tasks/QuickPlayTask.bs`, `ItemGrid/*`, `liveTv/*`) from blocking `fetchRes` to `await fetchAsync`; **(b)** parallelize each orchestrator's *independent* fetches via `await promises.all([...])` — a deferred perf opportunity (e.g. `home/LoadItemsTask`'s ~22 fetches) that today's `wait2` machinery makes not worth the regression risk; **(c)** non-pool HTTP (fonts / captions / SSDP via raw `roUrlTransfer`) — **resolved 2026-06-06: declined**, decoupled from the async/await trigger. These three are Task-thread datagram/binary/time-budgeted flows, not render-thread pool consumers, so they stay blocking Tasks with no generic wrapper (rationale: wrong layer / `&hf3` regression risk on `captionTask` / no shared shape + no auto-abandon fit — see decision `non-pool-http-stays-task-blocking`). Only items (a)/(b) remain open, against the async/await trigger. Until then the split is correct; do NOT preemptively rewrite working orchestrators into `wait2` promise-loops (negative readability, regression risk on hot paths).
