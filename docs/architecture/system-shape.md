...
- **PR-time (GitHub Action)**: [`journal-sync-precheck.yml`](../../.github/workflows/journal-sync-precheck.yml) runs two checks on `opened`/`edited`/`synchronize` — it spell-checks the candidate `progress.md` bullet the PR title will become, and [`pr-body-check.js`](../../scripts/lint/pr-body-check.js) FAILs a PR whose description is still an unfilled template, or that writes another repo's issue as the shorthand `repo#N`, which GitHub leaves unlinked (a bare `#N` always links to this repo, so it can't be judged from text; `/pr` resolves those with `--list-refs` before posting). The second exists because the repo merges **squash-only with `squash_merge_commit_message=PR_BODY`**: a PR description is not a courtesy to reviewers, it is the permanent commit message of everything on `main`, so a blank section lands in `git log` forever. Both skip on the same predicate `journal-sync` uses (bot authors, `dependencies` / `documentation` / `ci` / `automated` labels), since we author neither those titles nor those bodies. `edited` is the load-bearing trigger: a body fixed in the GitHub UI re-runs the gate with no new push.
