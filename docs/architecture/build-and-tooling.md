...
- **[`eslint.config.js`](../../eslint.config.js)** — flat config, `@eslint/js` recommended + `eslint-plugin-n` `mixed-esm-and-cjs` preset. Disables `n/no-unpublished-*` (false positive — JellyRock isn't an npm package). Enforces `n/hashbang` for CLI shebangs, `n/prefer-node-protocol` for ESM imports of built-ins, plus standard `no-unused-vars` / `no-var` / `prefer-const`. `eslint-config-prettier` runs LAST to disable formatting rules that would fight Prettier.
  - It also carries **path-scoped project invariants**, not just generic hygiene — a `files:` block plus `no-restricted-syntax` turns a convention that was previously prose-only into a gate. Current member: RTA waits (`tests/rta/lib/nav.js`, `tests/rta/lib/steps.js`, `tests/rta/demos/**`) must throw through `diagnosedError` rather than a bare `new Error`, so a timeout reports the device state it saw ([rta-tests.md](../dev/rta-tests.md#when-a-wait-times-out-it-reports-what-it-saw)). This is the cheap shape for a single-file-scope invariant: no new script, no `package.json` entry, and no CI wiring — it rides `lint:js`, which already has a CI home whose path filter matches, so [`ci-parity-check`](../../scripts/lint/ci-parity-check.js) stays satisfied. Prefer it over a bespoke `scripts/lint/*` checker unless the rule needs to reason across files. A legitimate exception disables the rule on its line **with a reason**.
