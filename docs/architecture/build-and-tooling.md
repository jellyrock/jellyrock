...
| `bsc-plugin-no-same-node-relaunch.cjs` **(error)** | A `launchTask(X)` in a function that earlier STOPs `X` — `X.control = "STOP"` in any casing, `X["control"]`, `X.setField("control", …)` or a literal `X.setFields({ control: … })` — or calls a same-file function that STOPs an `m.`-rooted `X` (one hop, e.g. `prepareDataLoad()`) | A new node assigned to `X` or a parent of it between the STOP and the launch clears it (the fix). A STOP after the launch (a timeout) and a STOP in another callback are not flagged. Known gaps: source order stands in for control flow (a STOP and a launch in exclusive branches are flagged); helpers are followed one hop, in one file, by bare name. `source/utils/tasks.bs` and `components/vendor/**` are excluded |
...
| `no-same-node-relaunch` | ✅ | ✅ | ❌ | **Deliberate**, same reasoning as the other Task rules. Put the reason after the code on the marker line |
