...
**Cross-file rules must clear before they re-derive.** A plugin whose finding depends on ONE file can register it and forget it — BSC clears a file's diagnostics by `location.uri` whenever that file re-validates. A plugin whose finding depends on TWO files cannot: editing the half that is not the diagnostic's anchor changes the verdict and clears nothing, so the stale finding survives until the anchor file is touched. Three plugins had this — `field-observer-wiring` (XML `onChange` + codebehind), `observe-without-on-destroy` (the XML says whether the component descends from `JRScreen`, the warning lands on the `.bs`) and `callfunc-interface` (the verdict lives in the target component's `<interface>`). The last was the worst: adding the `<function>` declaration the error asks for did not clear the error.
