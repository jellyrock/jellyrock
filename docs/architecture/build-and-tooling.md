...
| `bsc-plugin-no-task-fanout.cjs` **(error)** | A `launchTask(...)` call lexically inside a `for` / `for each` / `while` body — one Task thread per iteration, so the concurrent count scales with server data. This is the shape epic #728 actually took (`HomeRows`' per-library latest-media fan-out, removed in PR #762) | The argument decides it, not the loop: a stable dotted path rooted at `m` (`m.LoadNextUpTask`, `m.view.loadLogoTask`) names ONE node however many times the loop turns and is allowed — that is the live shape in `HomeRows.startParallelLoads()`. A loop variable, a call result, or an indexed step (`m.tasks[i]`) is flagged; so is a stable path the loop body **rebinds** (`m.loader = createObject(…)` then `launchTask(m.loader)`), which is a fresh node per turn wearing a stable name — and the first thing someone reaches for to clear the diagnostic. Rebinding a *parent* of the path counts; writing a *child* of it (configuring the node) does not. `source/utils/tasks.bs` and `components/vendor/**` are excluded. Known gaps, both interprocedural: a loop calling a helper that launches internally, and a local aliased to a stable slot before the loop (flagged though safe). Deliberate over-reporting: a literal key (`m["loader"]`) and a lazily-initialized singleton are flagged though stable — one suppression each, and neither occurs today |
