...
Eight plugins encode unwritten conventions documented in `components/CLAUDE.md` / `source/CLAUDE.md` so violations surface as IDE diagnostics + CI failures instead of bugs at runtime, and never crash the build on edge cases. The first four emit warnings (severity 2); `callfunc-interface`, `no-raw-run`, `no-task-fanout` and `no-hand-rolled-dialog` emit an **error** (severity 1) — an undeclared `callFunc` target is a guaranteed silent no-op, both Task-launch rules guard the `&h29` crash class from epic #728, and a hand-rolled dialog is invisible to the overlay machinery that supersedes and cancels dialogs, so none is a style nit.
