...
**The third Task rule, `no-same-node-relaunch`, is about a launch that does not happen.** Stopping a Task node and relaunching the same node in one callback, while its function still runs, is a race measured on device: the launch is sometimes silently ignored ([threading.md](threading.md#measured-findings)). A run that can restart needs a new node each time. The 16 app sites that predate the rule (2026-09-17) are listed in the plugin's `PENDING_MIGRATIONS` rather than suppressed inline, so there is no marker to copy onto a new site. **The list can only shrink, and that is enforced rather than hoped for: every state an entry can be in raises an actionable error except the one where it matches its site.** That takes two hooks, because the states divide by what each can see. `afterValidateFile` answers *did this entry's site fire* — matched, or the function is there and does not relaunch that node, or no function of that name is there. `afterValidateProgram` answers *can this entry be checked at all* — its file may be missing from the build, be one the rule never inspects (`vendor/`, the launch wrapper), be spelled in a different case, or the entry may be listed twice. **None of the second group is visible to the file hook**, which never runs for such a file, so without that half the entry *and the real violation under it* both go quiet — the failure mode this design exists to prevent, arriving through the list meant to prevent it. A stale entry yields one claim and never a second contradicting it about the same node: the plugin sees only what the code does now, so it cannot distinguish a finished migration from an entry that was always wrong, and asserts neither. The sites are migrated one at a time, with a device check each, as tracked in `docs/progress.md`.
