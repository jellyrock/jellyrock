...
**`dts` is excluded on evidence that no probe can express, which is why the absence is a test rather than a comment.** `CanDecodeAudio` answers true for `dts` and the device really can decode it — but Roku's Audio node cannot *demux* a raw DTS elementary stream. A `.dts` errors `-5 "malformed data"` under all eight `streamFormat` values tried (`dts`, unset, `dca`, `mp4`, `mkv`, `wav`, `bogusformat` — and `mp3`, which "succeeds" only by handing the bytes to `mpg123` and finishing instantly), while the IDENTICAL bitstream muxed into `.mka` plays on the same device seconds later reporting `audioFormat=dts`. So `dts` belongs in the `mkv` row, where it already is, and a container row of its own would ship broken playback in place of the working transcode it replaced. `truehd` is excluded for an unrelated reason: Jellyfin does not index a `.thd` file at all — it sits in a scanned library folder and produces no item — so there is no standalone TrueHD container for a profile to match. Two corrections land alongside: the standing claim that Roku cannot play standalone `opus` was false and was costing the audio `mkv` row a codec the video `mkv` row kept, and container-scoped probes for `mat` are now refused rather than sent, because Roku answers them `{ container: ["mat"], updated: "container" }` — a rejection the old code misread as a real "no".
