...
**Extended 2026-08-30 to the remaining audio containers, on generated test media.** `wav`, `aiff`, elementary `.aac`, `ogg` and `mka` now ship too, each gated on a real `CanDecodeAudio` probe and each verified end to end at `PlayMethod=DirectPlay` on a Streaming Stick 4K. Three things only real files could settle. **The container string that matters comes from `/PlaybackInfo`, not `/Items`** — the latter reports a shorter value (`m4a`, `mov`) and designing against it is a trap; `/PlaybackInfo` reports the full container list its probe reports, so `mov,mp4,m4a,3gp,3g2,mj2` covers `.m4a` and `.m4b` alike and the single token `m4a` matches both. **An `.mka` reports `mkv`, and `.oga` / `.opus` both report `ogg`** — which is why no client change was needed: `items.bs` already sets `streamFormat` from the container, and `ogg` is exactly the value the device requires (it errors `-5 "malformed data"` on an empty or wrong one, the same way `flac` does). **`wav` and `aiff` must stay open to any codec** because Jellyfin reports `pcm_s16le` / `pcm_s24le` / `pcm_s16be`, none of which equals the `pcm` the profile can probe, so an `AudioCodec` list would match nothing. Two probe answers were also measured WRONG and overridden by playing the file: `mp3`-in-`wav` reports false yet plays (via Roku's `mpg123` decoder), and the standing comment that "Opus audio files are not supported by Roku" is false — `.opus` and opus-in-`ogg` both play, so that skip was removed and standalone opus now carries the same channel ceiling as opus in video.
