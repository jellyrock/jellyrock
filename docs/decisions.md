...
A library grid whose first page (or Genres list) fails shows its own failure state with a Try again the USER presses; it never retries on its own. The query that failed may still be running on the server — #869's slow count is the case that prompted this — and an unprompted retry stacks a second copy on it. A failed later page keeps the items shown and the known total, raises one toast until a page loads, and is asked for again when focus next moves near the end or the user comes back to the grid from another screen — both user-driven, one request at a time (`m.loading`). While another screen covers the grid it neither toasts nor moves focus; those rules are `gridPaging`'s, unit-tested. Focus landing back in the grid already asked on return as a side effect (the grid re-reports its focused item; measured 2026-09-25, Ultra, Roku OS 15.3.4), but only when focus returns there — the explicit resume makes it the rule. `LoadItemsTask2.status` is `ok`/`failed` per `latest-rows-failure-vs-empty`; the cause goes to the log (`apiResponse.jsonFailure`), not to the screen.
