...
- **A run that can restart while the previous one is in flight needs a NEW Task node.** A STOP then `launchTask` of the same node in one function (or after a same-file helper that STOPs it) is a build error (`no-same-node-relaunch`): while the old function still runs, the relaunch is sometimes silently ignored. So is a `launchTask(m.x)` with no new node assigned first, which is ignored every time the previous run is still going. Use `replaceTask()` / `isCurrentTaskEvent()` from `source/utils/tasks.bs`; the full rule is in [`components/CLAUDE.md`](../components/CLAUDE.md).
